...
class SimulationResults:
    ...
    def __init__(self, n_steps: int):
        ...
        self.data: Dict[str, np.ndarray] = {
            ...
            'weather': {
                'I_glob': np.zeros(n_steps)
            },
            'I_crop': np.zeros(n_steps)  # 작물 위 광량 [W/m²]
        }
    ...
    def update(self, step: int, state: Dict) -> None:
        ...
        # 일사량 저장
        if 'I_glob' in state:
            self.data['weather']['I_glob'][step] = state['I_glob']
        
        if 'I_crop' in state:
            self.data['I_crop'][step] = state['I_crop']
...
def plot_results(results: SimulationResults, save_path: Optional[str] = None) -> None:
    ...
    # 3. 온실 밖 온도 & 일사량 & 작물 위 광량 그래프
    ax3 = plt.subplot(4, 2, 3)
    ax3.plot(results.times, results.data['temperatures']['outdoor'], label='Outdoor Temp [°C]')
    if 'weather' in results.data and 'I_glob' in results.data['weather']:
        ax3.plot(results.times, results.data['weather']['I_glob'], label='Global Radiation [W/m²]')
    else:
        ax3.plot(results.times, np.zeros_like(results.times), label='Global Radiation [W/m²] (no data)')
    ax3.plot(results.times, results.data['I_crop'], label='Crop Light (Sun + Supplemental) [W/m²]')
    ax3.set_title('Outdoor Temperature, Global Radiation, Crop Light')
...
def simulate_greenhouse(config: Optional[SimulationConfig] = None) -> SimulationResults:
    ...
    try:
        ...
        # 온실 모델 생성
        greenhouse = Greenhouse_2(time_unit_scaling=config.time_unit_scaling)
            ...
            try:
                ...
                if i % config.debug_interval == 0:
                    ...
                    logging.info(
                        ...
                        f"Tout={state['temperatures']['outdoor']:.2f}°C, "
