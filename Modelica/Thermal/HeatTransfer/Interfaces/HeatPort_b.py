...
class HeatPort_b(HeatPort):
    """
    Modelica의 HeatPort_b 인터페이스
    
    이 클래스는 Modelica의 HeatPort_b 커넥터를 구현합니다.
    HeatPort를 상속받으며, 채워지지 않은 사각형 아이콘을 가진 1차원 열전달 포트입니다.
    ...
    def __str__(self):
        """열전달 포트의 문자열 표현"""
        return f"HeatPort_b(T={self.T:.2f}K, Q_flow={self.Q_flow:.2f}W)"
