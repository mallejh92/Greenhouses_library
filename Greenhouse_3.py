...
class Greenhouse_3:
    ...
    def __init__(self, time_unit_scaling=1.0):
        ...
        # Initialize components with proper initial temperatures
        initial_T_air = 18.0 + 273.15  # 18°C
        initial_T_floor = 16.0 + 273.15  # 16°C
        initial_T_canopy = 17.0 + 273.15  # 17°C
        initial_T_cover = 12.0 + 273.15  # 12°C
        initial_T_screen = 17.0 + 273.15  # 17°C
        ...
        # Ventilation heat and mass flows (updated each step)
        self.Q_flow_AirOut = 0.0
        self.MV_flow_AirOut = 0.0
        self.Q_flow_TopOut = 0.0
        self.MV_flow_TopOut = 0.0

        # CO2 injection parameters
        self.phi_ExtCO2 = 27  # External CO2 injection rate [mg/(m²·s)]

        # Initialize components with proper initial conditions
        from Components.Greenhouse.Cover import Cover
        from Components.Greenhouse.Air import Air
        from Components.Greenhouse.Canopy import Canopy
        from Components.Greenhouse.Floor import Floor
        ...
        self.cover = Cover(
            ...
            T_start=initial_T_cover  # Set initial temperature
        )
        ...
        self.air = Air(
            ...
            steadystateVP=True,
            h_Air=3.8,
            T_start=initial_T_air  # Set initial temperature
        )
        ...
        self.canopy = Canopy(
            ...
            steadystate=True,
            LAI=1.06,
            T_start=initial_T_canopy  # Set initial temperature
        )
        ...
        self.floor = Floor(
            ...
            V=0.01*self.surface,
            steadystate=True,
            T_start=initial_T_floor  # Set initial temperature
        )
        ...
        self.thScreen = ThermalScreen(
            ...
            SC=0,
            steadystate=False,
            T_start=initial_T_screen  # Set initial temperature
        )
        
        # Initialize heat ports with proper temperatures
        ...
        # Initialize mass ports with proper vapor pressures
        self.air.massPort.VP = self.air.VP
        self.air_top.massPort.VP = self.air_top.VP
        self.canopy.massPort.VP = 1500  # Initial VP
        self.floor.massPort.VP = 1000   # Initial VP
        self.cover.massPort.VP = 1200   # Initial VP
        self.thScreen.massPort.VP = 1400  # Initial VP
    ...
    def step(self, dt, time_idx):
        ...
        for _ in range(sub_steps):
            ...
            # 2. 구성요소 상태 업데이트 (파이프, 태양모델 등)
            self._update_components(sub_dt, current_weather, current_sp)
            
            # 3. 현재 포트 온도와 시야계수 갱신
            self._update_port_connections_ports_only(sub_dt)

            # 4. 열전달(복사, 대류) 및 환기 흐름 계산
            self._update_heat_transfer(sub_dt)
            
            # 5. 계산된 열유량을 각 컴포넌트에 전달하고 적분
            self.cover.set_inputs(self.cover.Q_flow, self.solar_model.R_SunCov_Glob)
            self.floor.set_inputs(self.floor.Q_flow, [self.solar_model.R_SunFlr_Glob,
                                                     self.illu.R_IluFlr_Glob])
            self.canopy.set_inputs(self.canopy.Q_flow, [self.solar_model.R_SunCan_Glob,
                                                       self.illu.R_IluCan_Glob])
            self.air.set_inputs(self.air.Q_flow, [self.solar_model.R_SunAir_Glob,
                                                 self.illu.R_IluAir_Glob])
            self.thScreen.set_inputs(self.thScreen.Q_flow)
            self.air_top.set_inputs(self.air_top.Q_flow)

            # Component states update
            self.cover.step(sub_dt)
            self.floor.step(sub_dt)
            self.canopy.step(sub_dt)
            self.air.step(sub_dt)
            self.thScreen.step(sub_dt)
            self.air_top.step(sub_dt)

            # 6. 포트 연결 재갱신 (다음 서브스텝 계산을 위해)
            self._update_port_connections_ports_only(sub_dt)
    ...
    def _update_components(self, dt, weather, setpoint):
        ...
        # Store ventilation flows and immediately apply heat exchange
        self.Q_flow_AirOut = Q_flow_AirOut
        self.MV_flow_AirOut = MV_flow_AirOut
        self.air.set_inputs(Q_flow=Q_flow_AirOut)
        self.air.massPort.MV_flow = MV_flow_AirOut
        self.air.step(dt)
        ...
        self.Q_flow_TopOut = Q_flow_TopOut
        self.MV_flow_TopOut = MV_flow_TopOut
        self.air_top.set_inputs(Q_flow=Q_flow_TopOut)
        self.air_top.massPort.MV_flow = MV_flow_TopOut
    ...
    def _update_heat_transfer(self, dt):
        """Update all port temperatures from component states"""
        self.air.heatPort.T = self.air.T
        self.air_top.heatPort.T = self.air_top.T
        self.cover.heatPort.T = self.cover.T
        self.canopy.heatPort.T = self.canopy.T
        ...
        # Update radiation port temperatures
        if hasattr(self, 'Q_rad_CanCov'):
            self.Q_rad_CanCov.port_a.T = self.canopy.T
            self.Q_rad_CanCov.port_b.T = self.cover.T
        
        if hasattr(self, 'Q_rad_FlrCan'):
            self.Q_rad_FlrCan.port_a.T = self.floor.T
            self.Q_rad_FlrCan.port_b.T = self.canopy.T
        
        if hasattr(self, 'Q_rad_FlrCov'):
            self.Q_rad_FlrCov.port_a.T = self.floor.T
            self.Q_rad_FlrCov.port_b.T = self.cover.T
        
        if hasattr(self, 'Q_rad_CanScr'):
            self.Q_rad_CanScr.port_a.T = self.canopy.T
            self.Q_rad_CanScr.port_b.T = self.thScreen.T
        
        if hasattr(self, 'Q_rad_FlrScr'):
            self.Q_rad_FlrScr.port_a.T = self.floor.T
            self.Q_rad_FlrScr.port_b.T = self.thScreen.T
        
        if hasattr(self, 'Q_rad_ScrCov'):
            self.Q_rad_ScrCov.port_a.T = self.thScreen.T
            self.Q_rad_ScrCov.port_b.T = self.cover.T
        
        if hasattr(self, 'Q_rad_CovSky'):
            self.Q_rad_CovSky.port_a.T = self.cover.T
            self.Q_rad_CovSky.port_b.T = self.T_sky
        
        # Update convection port temperatures
        if hasattr(self, 'Q_cnv_CanAir'):
            self.Q_cnv_CanAir.port_a.T = self.canopy.T
            self.Q_cnv_CanAir.port_b.T = self.air.T
        
        if hasattr(self, 'Q_cnv_FlrAir'):
            self.Q_cnv_FlrAir.port_a.T = self.floor.T
            self.Q_cnv_FlrAir.port_b.T = self.air.T
        
        if hasattr(self, 'Q_cnv_CovOut'):
            self.Q_cnv_CovOut.port_a.T = self.cover.T
            self.Q_cnv_CovOut.port_b.T = self.T_out
