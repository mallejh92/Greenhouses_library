...
class CombiTimeTable:
    ...
    def get_value(self, time: float, interpolate: bool = False) -> Union[float, List[float]]:
        ...
        if interpolate:
            ...
            result = [self.data.loc[idx, f'col_{i}'] for i in self.columns]
...
class Greenhouse_1:
    ...
    def _init_flows(self) -> None:
        ...
        # 2. 대류 및 증발/응결 시스템 초기화
        self.Q_conv_AirScr = Convection_Evaporation(
            A=SURFACE_AREA,
            SC=self.thScreen.SC
        )
        
        self.Q_conv_AirCov = Convection_Condensation(
            phi=self.cover.phi,
            A=SURFACE_AREA,
            floor=False,
            thermalScreen=self.thScreen,
        ...
        self.Q_conv_TopCov = Convection_Condensation(
            phi=self.cover.phi,
            A=SURFACE_AREA,
            floor=False,
            thermalScreen=self.thScreen,
        ...
        self.Q_conv_AirFlr = Convection_Condensation(
            phi=self.cover.phi,
            A=SURFACE_AREA,
            floor=True,
            thermalScreen=self.thScreen,
        ...
        self.Q_rad_LowFlr = Radiation_N(
            ...
            epsilon_b=0.95   # 파이프 방사율
        )
        
        self.Q_rad_LowCan = Radiation_N(
            N=5,
            A=SURFACE_AREA,
            epsilon_a=0.95,  # 작물 방사율
            epsilon_b=0.95
        )
        ...
        # 10. 작물 자유 대류 초기화
        self.Q_conv_CanAir = CanopyFreeConvection(
            A=SURFACE_AREA,
            LAI=self.canopy.LAI
        )
        
        # 11. 외부 공기 대류 초기화
        self.Q_conv_OutAir = OutsideAirConvection(
            A=SURFACE_AREA,
            u=self.u_wind,
            phi=self.cover.phi
        )
        ...
        # 12. 외부 공기 자유 대류 초기화
        self.Q_conv_OutAirFree = FreeConvection(
            A=SURFACE_AREA,
            phi=self.cover.phi
        )
    ...
    def step(self, dt: float, time_idx: int) -> None:
        ...
        for _ in range(max(1, int(dt / 60))):
            ...
            if not self._verify_state():
                raise ValueError("상태 검증 실패: 열 균형이나 수증기 균형이 맞지 않습니다.")
    ...
    def _update_components(self, dt: float, weather: List[float], setpoint: List[float]) -> None:
        ...
        # 6. 조명 제어 업데이트
        self.illu.switch = weather[9]  # 조명 ON/OFF 신호
    ...
    def _update_heat_ports(self) -> None:
        """열 포트 연결을 업데이트합니다."""
        # 공기와 외피 사이의 열 포트
        self.Q_conv_AirCov.heatPort_a.T = self.air.T
        self.Q_conv_AirCov.heatPort_b.T = self.cover.T
        
        # 공기와 보온 스크린 사이의 열 포트
        self.Q_conv_AirScr.heatPort_a.T = self.air.T
        self.Q_conv_AirScr.heatPort_b.T = self.thScreen.T
        
        # 상부 공기와 외피 사이의 열 포트
        self.Q_conv_TopCov.heatPort_a.T = self.air_Top.T
        self.Q_conv_TopCov.heatPort_b.T = self.cover.T
        
        # 보온 스크린과 상부 공기 사이의 열 포트
        self.Q_conv_AirScr.heatPort_a.T = self.thScreen.T
        self.Q_conv_AirScr.heatPort_b.T = self.air_Top.T
        
        # 작물과 공기 사이의 열 포트
        self.Q_conv_AirFlr.heatPort_a.T = self.floor.T
        self.Q_conv_AirFlr.heatPort_b.T = self.air.T
        
        # 외피와 외부 공기 사이의 열 포트
        self.Q_conv_AirCov.heatPort_a.T = self.cover.T
        self.Q_conv_AirCov.heatPort_b.T = self.T_out
    
    def _update_mass_ports(self) -> None:
        """질량 포트 연결을 업데이트합니다."""
        # 공기와 외피 사이의 질량 포트
        self.Q_conv_AirCov.massPort_a.VP = self.air.massPort.VP
        self.Q_conv_AirCov.massPort_b.VP = self.cover.massPort.VP
        
        # 공기와 보온 스크린 사이의 질량 포트
        self.Q_conv_AirScr.massPort_a.VP = self.air.massPort.VP
        self.Q_conv_AirScr.massPort_b.VP = self.thScreen.massPort.VP
        
        # 상부 공기와 외피 사이의 질량 포트
        self.Q_conv_TopCov.massPort_a.VP = self.air_Top.massPort.VP
        self.Q_conv_TopCov.massPort_b.VP = self.cover.massPort.VP
        
        # 보온 스크린과 상부 공기 사이의 질량 포트
        self.Q_conv_AirScr.massPort_a.VP = self.thScreen.massPort.VP
        self.Q_conv_AirScr.massPort_b.VP = self.air_Top.massPort.VP
    ...
    def _update_radiation_ports(self) -> None:
        """복사 포트 연결을 업데이트합니다."""
        # 작물과 외피 사이의 복사 포트
        self.Q_conv_AirCov.port_a.T = self.canopy.T
        self.Q_conv_AirCov.port_b.T = self.cover.T
        
        # 작물과 보온 스크린 사이의 복사 포트
        self.Q_conv_AirScr.heatPort_a.T = self.canopy.T
        self.Q_conv_AirScr.heatPort_b.T = self.thScreen.T
        
        # 바닥과 작물 사이의 복사 포트
        self.Q_conv_AirFlr.port_a.T = self.floor.T
        self.Q_conv_AirFlr.port_b.T = self.canopy.T
        
        # 바닥과 외피 사이의 복사 포트
        self.Q_conv_AirCov.port_a.T = self.cover.T
        self.Q_conv_AirCov.port_b.T = self.T_out
        
        # 바닥과 보온 스크린 사이의 복사 포트
        self.Q_conv_AirScr.heatPort_a.T = self.floor.T
        self.Q_conv_AirScr.heatPort_b.T = self.thScreen.T
        
        # 보온 스크린과 외피 사이의 복사 포트
        self.Q_conv_AirCov.port_a.T = self.cover.T
        self.Q_conv_AirCov.port_b.T = self.T_out
        
        # 외피와 하늘 사이의 복사 포트
        self.Q_conv_AirCov.port_a.T = self.cover.T
        self.Q_conv_AirCov.port_b.T = self.T_sky
    ...
    def _calculate_convection(self) -> None:
        """대류 열전달을 계산합니다."""
        # 공기와 외피 사이의 대류
        self.Q_conv_AirCov.step()
        
        # 공기와 보온 스크린 사이의 대류
        self.Q_conv_AirScr.step()
        
        # 상부 공기와 외피 사이의 대류
        self.Q_conv_TopCov.step()
        
        # 보온 스크린과 상부 공기 사이의 대류
        self.Q_conv_AirScr.step()
        
        # 작물과 공기 사이의 대류
        self.Q_conv_AirFlr.step()
    ...
    def _calculate_component_heat_balance(self) -> None:
        ...
        # 공기 열 균형
        self.air.Q_flow = (
            self.Q_conv_AirCov.Q_flow +
            self.Q_conv_AirScr.Q_flow +
            self.Q_conv_AirFlr.Q_flow +
            self.Q_cnv_LowAir.Q_flow +
            self.Q_cnv_UpAir.Q_flow +
            self.Q_ven_AirOut.Q_flow +
            self.Q_ven_AirTop.Q_flow  # 스크린을 통한 공기 흐름 열량 추가
        )
        
        # 상부 공기 열 균형
        self.air_Top.Q_flow = (
            self.Q_conv_TopCov.Q_flow +
            self.Q_conv_AirScr.Q_flow +
        ...
        self.thScreen.Q_flow = (
            ...
            self.Q_conv_AirCov.Q_flow +
            self.Q_conv_AirScr.Q_flow +
            self.Q_rad_LowScr.Q_flow +
            self.Q_rad_UpScr.Q_flow
        )
    ...
    def _update_illumination_control(self, weather: List[float]) -> None:
        """조명 제어를 업데이트합니다."""
        # 조명 스위치 상태 업데이트
        self.illu.switch = weather[9]  # 조명 ON/OFF 상태 [0-1]
    ...
    def _verify_temperature_ranges(self) -> None:
        ...
        for name, temp in temps.items():
            if not (MIN_TEMPERATURE - 273.15 <= temp <= MAX_TEMPERATURE - 273.15):
