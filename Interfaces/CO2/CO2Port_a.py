...
class CO2Port_a(CO2Port):
    """
    CO2 port for 1-dim. CO2 transfer (filled rectangular icon)
    
    This class extends CO2Port with a filled rectangular icon representation.
    ...
    def __init__(self, CO2: float = 0.0, MC_flow: float = 0.0, name: str = "port_a"):
        """
        Initialize CO2Port_a
        
        Args:
            CO2 (float): Initial CO2 concentration in mg/m³ (default: 0.0)
            MC_flow (float): Initial CO2 flow rate in mg/(m²·s) (default: 0.0)
            name (str): Component name (default: "port_a")
        """
    ...
    def __str__(self) -> str:
        ...
        return (f"CO2Port_a(name={self.name})\n"
